...
describe("tag operations", () => {
  it("validates, maps, and formats get", () => {
    ...
    assert.match(operation.format({ result: true, items: [{ _id: "docs", count: 5 }, { _id: "read-later", count: 3 }] }), /Found 2 tag\(s\)\.[\s\S]*1\. docs \(5\)[\s\S]*2\. read-later \(3\)/);
  });

  it("validates, maps, and formats rename", () => {
    const operation = op("rename");
    assertInvalid(operation.validate({ action: "rename", tags: [] }), /exactly one tag/);
    assertInvalid(operation.validate({ action: "rename", tags: ["doc", "docs"] }), /exactly one tag/);
    assertInvalid(operation.validate({ action: "rename", tags: ["doc"] }), /requires replace/);
    ...
    assert.match(operation.format({ result: true, modified: 1 }), /Updated tag\(s\)\./);
  });

  it("validates, maps, and formats merge", () => {
    const operation = op("merge");
    assertInvalid(operation.validate({ action: "merge", tags: [] }), /at least 1 tag/);
    assertInvalid(operation.validate({ action: "merge", tags: ["old1"] }), /requires replace/);
  ...
  it("validates, maps, and formats remove", () => {
    ...
    assertInvalid(operation.validate({ action: "remove", tags: [] }), /at least 1 tag/);
